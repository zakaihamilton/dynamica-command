...
export function drawTurretCannon(
  ...
  // Dynamic turntable well shadow under the head
  ctx.save();
  ...
  // Subtle directional shadow cast by the elevated turret housing
  ...
  const perpY = cos * barrelSpread * 0.5; // isometric vertical compression
  ...
  // Targeting laser guides when aiming at an enemy target
  if (target && target.hp > 0) {
    const b = tileToScreen(target.x, target.y, cam, entityElev(state, target));
    const targetY = b.y + 6 * z;
  ...
  // Twin Muzzle flash & thermal bloom when firing
  if (isFiring) {
    ctx.save();
