...
import { renderMissionMusic } from "./music";
...
export async function supportsM4aExport(): Promise<boolean> {
  ...
  if (!Encoder || !AudioDataClass || typeof window === "undefined" || typeof window.OfflineAudioContext === "undefined") return false;
  try {
...
async function encodeM4a(buffer: AudioBuffer, onProgress?: (progress: MusicExportProgress) => void): Promise<Blob> {
  const Encoder = getEncoder();
  const AudioDataClass = getAudioData();
  ...
  try {
    ...
    for (let start = 0; start < totalFrames; start += framesPerChunk) {
      ...
      onProgress?.({ phase: "encoding", progress: Math.min(0.99, (start + frameCount) / totalFrames) });
