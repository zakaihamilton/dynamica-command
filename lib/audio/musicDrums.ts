...
export function playKick(audio: AudioGraphContext, g: MusicGraph, time: number, velocity: number): void {
  ...
  playNoise(audio, g.rhythmBus, time, 0.08 * velocity, drum.snareNoise * 1.45, 0.02, "highpass", drum.noisePan);
}
...
export function playSnare(audio: AudioGraphContext, g: MusicGraph, time: number, velocity: number, accent: boolean): void {
  ...
  playNoise(audio, g.rhythmBus, time, dry * 0.55, drum.snareNoise * 2.2, 0.035, "highpass", drum.noisePan);
  ...
  playNoise(audio, g.reverbSend, time, dry * 0.7, drum.snareNoise * 0.84, accent ? 0.2 : 0.14, "bandpass", drum.noisePan);
}
...
export function playHat(audio: AudioGraphContext, g: MusicGraph, time: number, velocity: number, open: boolean): void {
  const drum = g.style.drum;
  playNoise(audio, g.rhythmBus, time, (open ? 0.06 : 0.036) * velocity, open ? drum.openHatFrequency : drum.hatFrequency, open ? 0.17 : 0.027, open ? "bandpass" : "highpass", open ? drum.noisePan + 0.12 : drum.noisePan - 0.08);
}
