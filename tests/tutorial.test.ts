...
describe("tutorial", () => {
  ...
  it("returns the produce prompt for the produce stage", () => {
    ...
    expect(tutorialPrompt(state)).toBe("Open Production (E) and queue infantry.");
  });

  it("returns the attack prompt for the attack stage", () => {
    const state = createTutorialMission();
    state.tutorialStage = "attack";
    expect(tutorialPrompt(state)).toBe("Use attack-move (Ctrl + right click) or attack an enemy unit.");
  });

  it("returns the repair prompt for the repair stage", () => {
    const state = createTutorialMission();
    state.tutorialStage = "repair";
    expect(tutorialPrompt(state)).toBe("Use the wrench (R) on a damaged structure.");
  });
