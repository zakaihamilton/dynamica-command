...
export function useGameCamera({
  ...
  const focusFromMinimap = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const px = Math.max(0, Math.min(mini.width - 1, (e.clientX - r.left) * (mini.width / r.width)));
    const py = Math.max(0, Math.min(mini.height - 1, (e.clientY - r.top) * (mini.height / r.height)));
    const tx = Math.max(0, Math.min(s.width - 1, Math.floor((px / mini.width) * s.width)));
    const ty = Math.max(0, Math.min(s.height - 1, Math.floor((py / mini.height) * s.height)));
